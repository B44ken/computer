...
export function assemble(source:string) {
  const image=new Uint8Array(64),used=new Set<number>(),labels:Record<string,number>=Object.create(null),listing:{address:number,byte:number,source:string,line:number}[]=[];
  const lines=source.split(/\r?\n/).map((text,i)=>({text,line:i+1}));
  const number=(t:string)=>/^0x[\da-f]+$/i.test(t)?parseInt(t,16):/^-?\d+$/.test(t)?Number(t):labels[t];
  function pass(emit:boolean) {
    let pc=0;
...
export function disassemble(byte:number) {return `${['lda','sta','sub','jz'][byte>>6]} 0x${(byte&63).toString(16).padStart(2,'0')}`;}
